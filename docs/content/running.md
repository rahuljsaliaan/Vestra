...
Before reading about *how* Outfit Buddy works, get it running so the explanations have something to point at. It takes about thirty seconds.

## Why you can't just double-click the file

Outfit Buddy is made of **ES modules** — JavaScript files that `import` from each other. For security reasons, browsers refuse to load modules from a `file://` address (the kind you get when you double-click `index.html`). You will see a blank page and a CORS error in the console.
...
When Outfit Buddy loads you should see:
